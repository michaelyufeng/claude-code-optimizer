...
| `/start --type [类型]` | 指定项目类型预设 (new/developing/existing/maintenance) ⭐ |
| `/status` | 查看状态 (含变更提示) |
| `/status --brief` | 简要状态 |
| `/detect` | 检测用户离线变更 |
| `/detect --sync` | 检测并自动同步 |
| `/scan` | 扫描分析已有项目，推荐工作模式 ⭐ v1.0.7 |
| `/scan --auto-start` | 扫描后自动启动推荐工作流 ⭐ v1.0.7 |
...
| `/reflect` | 整理到长期记忆 |
| `/reflect --show` | 查看长期记忆 |
| `/git-memory` | Git 历史作为记忆源 |
| `/hierarchy` | 管理多层级 CLAUDE.md 文档 |
...
| `/sprint --status` | 查看 Sprint 状态和看板 |
| `/sprint --update` | 每日站会更新 |
| `/sprint --complete` | 结束当前 Sprint |
| `/sprint --retrospective` | Sprint 回顾会议 |
| `/sprint --history` | 查看 Sprint 历史 |

### 🔧 开发模式 ⭐ v1.0.7

| 命令 | 说明 |
|------|------|
| `/developing` | 开发中模式 (增量开发，结构保护) |
...
/start --type developing    # 开发中项目，增量开发 (4 阶段) ⭐ v1.0.7
